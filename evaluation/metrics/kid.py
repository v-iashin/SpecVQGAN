...
def calculate_kid(featuresdict_1, featuresdict_2, subsets, subset_size, degree, gamma, coef0,
                  rng_seed, feat_layer_name):
    ...
    if subset_size > len(features_2):
        print(f'WARNING: subset size ({subset_size}) is larger than feature length ({len(features_2)}). ',
              'Using', len(features_2), 'for both datasets')
        subset_size = len(features_2)
    if subset_size > len(features_1):
        print(f'WARNING: subset size ({subset_size}) is larger than feature length ({len(features_1)}). ',
    ...
    for i in tqdm(range(subsets), leave=False, unit='subsets', desc='Computing Kernel Inception Distance'):
...
def polynomial_mmd(features_1, features_2, degree, gamma, coef0):
    K_XX = polynomial_kernel(features_1, features_1, degree=degree, gamma=gamma, coef0=coef0)
    K_YY = polynomial_kernel(features_2, features_2, degree=degree, gamma=gamma, coef0=coef0)
    K_XY = polynomial_kernel(features_1, features_2, degree=degree, gamma=gamma, coef0=coef0)
